...
module usbfs_transaction #(
    parameter [9:0] ENDP_00_MAXPKTSIZE = 10'd32,    // 0端点数据包最大长度
...
reg        endp_00_data1 = '0;
reg [ 9:0] endp_00_count = '0;  //低速设备允许的最大数据有效载荷大小为 8 个字节。 全速设备的最大数据有效载荷大小为 1023. 高速设备的最大数据有效载荷大小为 1024 个字节。
...
always @ (posedge clk)
    if(~rstn) begin
        ...
        if(tp_byte_req) begin
            ...
            end else if(endp == 4'd1) begin //非零端点
                if(endp_81_count != '0 && in_valid) begin
                    endp_81_count <= endp_81_count - 10'd1;
                    tp_fin_n <= 1'b1;   //指示新的数据填充，0表示未填充，1表示已填充
                    tp_byte <= in_data;
                end
            end
        end
