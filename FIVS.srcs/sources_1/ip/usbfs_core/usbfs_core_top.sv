...
//-------------------------------------------------------------------------------------------------------------------------------------
localparam RESET_CYCLES = 60_000_000;  // 1000ms
...
always @ (posedge clk or negedge rstn)
    if(~rstn) begin
        ...
        if(usb_rstn_cnt < RESET_CYCLES) begin   //reset计时器
            usb_dp_pull <= 1'b0;
            usb_rstn <= 1'b0;
...
// USB D+ and D- driver
//-------------------------------------------------------------------------------------------------------------------------------------
assign usb_dp = usb_oe ? usb_dp_tx : 1'bz;  // usb_oe : tri-state direct control signal of USB-D+ and USB-D-,  1:device TX,  0:device RX
